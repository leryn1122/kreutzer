...
func (hook *GitHubWebhook) Verify(secret []byte) error {
	if !hook.SignedBy(secret) {
		...
		return errors.New(ErrMsgInvalidSignature)
	}
	return nil
}

func (hook *GitHubWebhook) GetHookID() string {
	return hook.HookID
}

func (hook *GitHubWebhook) GetChannel() string {
	return "github"
}

func (hook *GitHubWebhook) GetChannelID() int {
	return GithubType
}
...
type GithubWebhookStrategy struct {
}

func (s *GithubWebhookStrategy) GetRawFile(repo, tag, path string) string {
	return fmt.Sprintf("https://raw.githubusercontent.com/%s/%s/%s", repo, tag, path)
}
